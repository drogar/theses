...
tensor in an inverse category. Conversely, it goes on to then show how a tensor, denoted as $\+$,
may be used to create a disjointness relation and a disjoint join in an inverse category  --- at
which point we can prove that $A\+B$ is a disjoint sum with respect to this structure.



\input{chapters/section/disjoint-sums}
...
\begin{definition}\label{def:disjointness_tensor}
  Given an inverse category \X with restriction zero and $\+$ a symmetric monoidal tensor, then $\+$
...
\begin{example}[\pinj has a disjointness tensor]\label{ex:pinj_has_disjointness_tensor}
  ...
  the disjoint union as a pair $(x,n)$ where $x$ is an element of the sets in the disjoint union
  and $n$ is the  order of the disjoint union. That is, when
  \[
...
\begin{lemma}\label{lem:zero_plus_zero_is_zero}
  Given an inverse category \X with restriction zero and $+$ a disjointness tensor, then the map
...
\begin{lemma}\label{lem:tensor_identities}
  Given an inverse category \X with restriction zero and $\+$ a disjointness tensor,
  then $\scpa = \icpa$ and $\scpb = \icpb$ and the following identities hold:
  \begin{enumerate}[{(}i{)}]
...
As the above proof showed $\scp{i} = \icp{i}$, the explicit notation of $\icp{i}$ will be used for the
remainder of this thesis.

\begin{corollary}\label{cor:split_plus_tensor}
  In an inverse category \X with a restriction zero and $\+$ a disjointness tensor, the following
  identities hold:
  \begin{multicols}{2}
    \begin{enumerate}[{(}i{)}]
      \item $\cpa (f \+g) \icpa = f$;
...
\begin{definition}\label{def:up_triangle_and_down_triangle}
  In an inverse category \X with a restriction zero and $\+$ a disjointness tensor, define a partial
...
\begin{lemma}\label{lem:properties_of_tjdown_and_tjup}
  ...
  following relations hold for $\tjdown$ and $\tjup$:
   \begin{enumerate}[{(}i{)}]
    ...
    \item Dually to \ref{lemitem:l_r_l_pull_right}, when $f \tjup g$ exists, $(h \+ k) f \tjup g = h f \tjup k g$.
...
\begin{proof}
  ...
  \begin{enumerate}[{(}i{)}]
    ...
    \item From the diagram for $\tjup$, we know:
      ...
      \]
      As $f\tperp g$, by \axiom{Dis}{2} $f\inv{g} = f \rg{g} \inv{g} = 0 \inv{g} = 0$ and therefore,
...
%   In an inverse category \X with a restriction zero and disjointness tensor, define two partial
%   operations on pairs of arrows in \X to another arrow in \X. First, for arrows
%   $f:A \to B$ and $g:A \to C$, suppose there are arrows $f':A  \to B\+X$ and $g': A \to C\+X$
%   such that $f'\icpa =f$, $g'\icpa = g$ and $f'\icpb = g' \icpb$. Then,  $f\gtjdown g$ if
...
%   $\alpha: A \to B\+C\+0$ exists and note that setting $f\tjdown g = \alpha \upr$ makes the
...
We are now set up to prove that it is possible to create a disjointness relation based on the
...
\begin{lemma}\label{lem:tensor_disjointness_is_disjointness}
  ...
  $f\tjup g$ exist.  If \X is an inverse category with a restriction zero and $\+$ a disjointness
  tensor then the relation $\tperp$ is a disjointness relation.
\end{lemma}
\begin{proof}
  The proof requires us to show that $\tperp$ satisfies the disjointness axioms. We will use
  \axiom{Dis}{6'} in place of \axiom{Dis}{6} and \axiom{Dis}{7} as discussed in
...
\begin{example}[$\tperp$ in \pinj]\label{ex:tensor_perp_in_pinj}
  ...
  Note that this is due to the actual choices of the tensor and $\perp$. If we had initially chosen
  a different $\perp$, such as $\perp_0$, where $f\perp_0 g$ iff $f$ or $g$ are $\zeroob$, then
  $\tperp \ne \perp_0$.
\end{example}
...
\begin{equation}
  ...
  }\label{eq:definition-of-f-tjoin-g}
\end{equation}

Using Lemma~\ref{lem:properties_of_tjdown_and_tjup}, this may be rewritten in a variety of
equivalent ways:
\begin{align}
  f \tjoin g &= (\rst{f}\tjdown\rst{g}) (f\+g)( \rg{f}\tjup\rg{g}) \label{eq:f-tjoin-g-1}\\
  &= (f \tjdown g)\, (\rg{f}\tjup\rg{g}) \label{eq:f-tjoin-g-2}\\
  & = (\rst{f}\tjdown\rst{g})\,  (f\tjup g) \label{eq:f-tjoin-g-3}\\
  & = (f \tjdown g) (\inv{f}\+\inv{g})(f\tjup g). \label{eq:f-tjoin-g-4}
\end{align}

In particular, note that $\rst{f}\tjoin \rst{g} = (\rst{f} \tjdown \rst{g})
(\rst{f}\tjup\rst{g})$ as $\rg{\rst{g}} = \rst{g}$.

\begin{example}[Disjoint sum tensor in \pinj]\label{ex:tensor_join_in_pinj}
  Using Equation~\ref{eq:f-tjoin-g-3} for $f\tjoin g$,  calculate:
  \begin{equation}
    \rst{f}\tjdown\rst{g}(a) =
    \begin{cases}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item [\axiom{DJ}{4}] This requires $\tperp [f,g,h]$ if and only if $f \tperp (g \tjoin h)$.
      ...
      h$. By \axiom{DJ}{1} $g, h \le g\tjoin h$ , as shown in this proof. This gives
...
\begin{lemma}\label{lem:disjoint_sum_tensor_preserves_the_disjoint_join}
  ...
  join. That is,
  \begin{align}
    f \tperp g, \ h \tperp k&\text{ implies } f\+ h \tperp g \+ k,
    \label{eq:disjoint_sum_preserve_perp}\\
    f \tperp g, \ h \tperp k &\text{ implies } (f\tjoin g)\+(h\tjoin k) = (f \+h )\tjoin (g\+k).
...
%todo - can we give separating examples of tensor - not disjointness, tensor disjointness but not
%disjoint sum?
...
\end{proposition}
\begin{proof}
  Proposition~\ref{prop:enough_disjoint_sums_make_a_symmetrict_monoidal_tensor} showed that when
  $\+$ is derived from the disjoint sum, it is a symmetric monoidal tensor. Therefore, it only
  remains to show that it is a disjointness tensor and that it satisfies
  Definition~\ref{def:disjoint_sum_tensor}, i.e., is a disjoint sum tensor as well.
