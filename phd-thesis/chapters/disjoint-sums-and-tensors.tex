...
This chapter introduces the disjoint sum and shows how it may be used to define a tensor in an
inverse category. Conversely, it goes on to then show how a tensor $\+$ may be used to create a disjointness
...
The next objective is to characterize when a tensor provides a disjoint sum. The first step will be
to show how a disjointness tensor, defined below, may be used to create a disjointness relation in
an inverse category. This is done by first determining when it is possible for maps to work
...
\begin{proof}
  ...
  \begin{enumerate}[{(}i{)}]
    ...
    \item The assumption is $f\tjdown g$ exists, therefore $f = (f\tjdown g)\icpa$ and
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[\axiom{Dis}{5}] Show that if $f\tperp g$ then $h f \tperp h g$ for any map $h$.
      ...
      \ref{lemitem:l_r_l_universal_r_stable},  $h f \tjdown h g$ exists.
    ...
    \item[\axiom{Dis}{6'}] This requires showing $f\tperp g$ if and only if $\rst{f} \tperp \rst{g}$
      ...
      Lemma~\ref{lem:properties_of_tjdown_and_tjup}, items \ref{lemitem:l_r_l_pull_right} and
