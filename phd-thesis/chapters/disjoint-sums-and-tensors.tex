...
In this chapter, we introduce the disjoint sum and show how it may be used to define a tensor in an
inverse category. Conversely, we then show how a tensor $\+$ may be used to create a disjointness
relation and a disjoint join in an inverse category  --- at which point we can prove that $A\+B$ is
a disjoint sum with respect to this structure.
...
Our next objective is to characterize when a tensor provides a disjoint sum. The first step will be
to show how a disjointness tensor, defined below, may be used to create a disjointness relation in
an inverse category. We will do this by first determining when it is possible for maps to work
separately on the components of the tensor. This ability to separate the action of the maps will
allow us to define when the restriction and range of functions are disjoint, and therefore when the
maps are disjoint.

%todo - can there be something more said here indicating it is a lot of work?

Suppose \X is an inverse category  with a restriction zero, and $\+$ is a tensor product on $\X$.
We are assuming the following naming for the standard monoidal tensor isomorphisms:
\begin{align*}
   \upl &: 0 \+ A \to A &
   \upr &: A \+ 0 \to A\\
   a_\+ &: (A \+ B) \+ C \to A \+ (B \+ C) &
...
our mechanism for defining disjointness and eventually the disjoint join of maps. The existence of
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[\axiom{Dis}{5}] Show that if $f\tperp g$ then $h f \tperp h g$ for any map $h$.
      Assuming $f\tjdown g$ exists, by  Lemma~\ref{lem:properties_of_tjdown_and_tjup}, item
      \ref{lemitem:l_r_l_universal_r_stable}, we know that $h f \tjdown h g$ exists.
      Assuming $f\tjup g$, by item \ref{lemitem:l_r_r_pull_left} of the same lemma,
...
on an inverse category. However, when we wish to extend this to a disjoint join, the two relations
are insufficient to prove $\axiom{DJ}{4}$, That is, we need to show that $\tperp [f,g,h]$ implies $f
\tperp (g \tjoin h)$.
...
\begin{example}[Disjoint sum tensor in \pinj]\label{ex:tensor_join_in_pinj}
  We will use the third equality of those above for $f\tjoin g$, i.e.,
  $\rst{f}\tjdown\rst{g}\,  f\tjup g$.
...
\begin{proof}
  \prepprooflist
  \begin{enumerate}[{(}i{)}]
    \item By Lemma~\ref{lem:disjointness_various} \ref{lemitem:djv_universal}, we know that
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item [\axiom{DJ}{4}] We need to show $\tperp [f,g,h]$ if and only if $f \tperp (g \tjoin h)$.
      ...
      h$. We also know $g, h \le g\tjoin h$ by \axiom{DJ}{4}, as shown in this lemma. This gives us
...
Finally, to complete the cycle, we have:
...
\end{proposition}
\begin{proof}
  In Proposition~\ref{prop:enough_disjoint_sums_make_a_symmetrict_monoidal_tensor} we showed that
  $\+$ was a symmetric monoidal tensor. Therefore we need only to show that it is a disjointness
  tensor that also satisfies Definition~\ref{def:disjoint_sum_tensor}.

  We first note that we immediately have that it is a   disjointness tensor as:
    \begin{enumerate}[{(}i{)}]
      \item We have shown above it is a restriction functor. (Proposition~\ref{prop:enough_disjoint_sums_make_a_symmetrict_monoidal_tensor})
      \item We have shown above that the unit is the restriction zero. (Proposition~\ref{prop:enough_disjoint_sums_make_a_symmetrict_monoidal_tensor})
