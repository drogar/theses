...
\begin{definition}\label{def:xt}
  ...
  \category{objects as in \X}
  {
    ...
    \ref{defn:xequivalence}) with the following structure in \X: %
    \[
...
\begin{proof}
  ...
  \ref{lem:mediating_map_equivalence_is_symmetric_reflexive_and_transitive}. The existence of the
  ...
  \ref{lem:identities_involving_restriction}. Then, calculating in \X, we have a mediating map of
  ...
  Lemma \ref{lem:identities_involving_restriction}. The diagram below
  \[
    \xymatrix @C=50pt @R=45pt{
...
\begin{proof}
  ...
  This is done by applying Lemma \ref{lem:identities_involving_restriction} once on the left and
  thrice on the right.
...
\begin{proof}
  ...
  \ref{lem:xt_is_a_category}, \ref{lem:xt_is_a_restriction_category},
...
\begin{proof}
  ...
  \ref{lem:properties_of_delta_and_tensor_in_a_discrete_inverse_category}, part \ref{le:restfg},
...
\subsection{Quantum computation} % (fold)
...
\begin{align*}
  ...
  ul_A &:A \to I \* A
\end{align*}
with standard coherence conditions, as in \cite{bib:categorieswrkmath:maclane1997}. Note that we
