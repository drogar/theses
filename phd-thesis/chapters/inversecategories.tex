...
the inverse setting. These we call \emph{inverse products}, which are defined in
sub-section~\ref{sub:inverse_products} below.
...
\begin{proof}
  ...
  \end{align}
  We will detail the proof of Equation~\eqref{eq:frobenius_righths_expressed_in_x}.
  Equation~\eqref{eq:frobenius_lefths_expressed_in_x} is proved similarly.
...
\begin{definition}\label{def:xt}
  ...
  {
    equivalence classes of maps (the equivalence class is defined below in
    Definition~\ref{defn:xequivalence}) with the following structure in \X: %
    \[
...
\begin{proof}
  ...
  First, we see $\restr{f (\inv{\usr}\*1) a_{\*}} = \restr{f}$ by
  Lemma~\ref{lem:restrictionvarious}. Then, calculating in \X, we have a mediating map of
  ...
  Lemma~\ref{lem:restrictionvarious}. The diagram below
  \[
    \xymatrix @C=50pt @R=45pt{
