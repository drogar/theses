...
The thesis does not, in general, consider resource usage or computability classes. For example,
although we do mention Turing machines in the introduction, we do not develop this further. In
particular the thesis does not consider whether the Cartesian Completion alters computability
classes of programs if it were to be applied to an inverse category whose maps consisted of
programs. The reader wishing to relate resource theory and the work in this thesis could use the
recent work by Coecke, Fritz and Spekkens\cite{coecke2014mathematical}, which defines a resource
theory as a symmetric monoidal category, as a starting point.

As another example, we define the concept of ``powerful'' in
Section~\ref{sec:partial_combinatory_algebras} when discussing partial combinatory algebras. In that
section, ``powerful'' does not relate to resource usage or a particular computability class, but
rather that an object $X$ contains copies of the objects products with itself, i.e., $X\times X,\
X\times X\times X\,\ldots$. An example of such an object is the integers.
...
desired answer is a significant problem worthy of study all on its
own\cite{neilsen2000:QuantumComputationAndInfo}.
