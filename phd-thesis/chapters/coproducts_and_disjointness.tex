...
Throughout this chapter and the following chapter we will work with a number of relations between
maps and operations on
...
\begin{theorem}[Cockett-Guo]\label{lem:join_determines_coproduct}
  Given a restriction category $\C$ with joins, then
  \[
...
\begin{example}[\pinj has a disjointness relation]\label{ex:pinj_has_a_disjointness_relation}
  ...
  $\rst{f}\intersect\rst{g}=\emptyset$ and $\rg{f}\intersect\rg{g} = \emptyset$. It is reasonably
  ...
  \begin{proof}
    ...
    \[
      \rst{f}\intersect\rst{g}=\emptyset \text{ and }\rg{h}\intersect\rg{k} = \emptyset.
    \]
    As discussed in Example~\ref{ex:pinj_has_meets}, we have
    $\rst{m n} \subseteq \rst{m}$ and that $\wrg{m n} \subseteq \rg{n}$. Hence we have
    \begin{align*}
      \rst{f h}\intersect\rst{g k}&\subseteq \rst{f}\intersect \rst{g}= \emptyset\\
      \wrg{f h}\intersect\wrg{g k}&\subseteq \rg{h}\intersect \rg{k}= \emptyset.
    \end{align*}
...
\begin{example}[\pinj has a disjoint join]\label{ex:pinj_has_a_disjoint_join}
  ...
  g$ means that $\rst{f}\intersect\rst{g} = \emptyset$ and $\rg{f}\intersect\rg{g} =
  \emptyset$. The empty meet of the restrictions means that $f\djoin g$ is still a partial function,
  as each $a''$ will appear only once. The empty meet of the ranges gives us that $f\djoin g$ is injective,
  because each $b''$ is unique.
...
\begin{example}[\pinj has a disjointness tensor]\label{ex:pinj_has_disjointness_tensor}
  In \pinj, the disjoint union, $\uplus$, is a disjointness tensor. Setting $\+$ as $\uplus$, we
  have the identity for the tensor is $\emptyset.
\begin{align*}
   \upl &: 0 \+ A \to A\\
   \upr &: A \+ 0 \to A\\
   a_\+ &: (A \+ B) \+ C \to A \+ (B \+ C) \\
