...
\begin{example}\label{ex:algebras-not-preserving-unit-as-restriction-cat}
  Given $\cS$ is a symmetric monoidal category, define $\mathbf{Copy}(\sC)$ as the category whose objects are
  the objects of $\cS$ which are commutative comonoids and semigroup homomorphisms, i.e., do not
  preserve the unit. As noted in \cite{cockett2002:restcategories1}, when there is a counit, $!:B\to
  I$, this is a restriction category, where the restriction is given by
  \[
  ...
  We may also consider commutative monoids with semigroup morphisms, $\mathbf{Copy}(\cS)^{op}$.

  We may also define This is the dual of a counital copy category as defined in
  \cite{cockettlack2004:restcategories3}. Theorem 5.2 from   \cite{cockettlack2004:restcategories3}
  shows that counital copy categories have a restriction, thus $\D$ has a corestriction, i.e., an
  operator on $f:A\to B$ with $\rst{f}:B\to B$ fulfilling the duals of the restriction axioms.

  For example, consider the category of commutative monoids in \Ab, the category of abelian
  categories. This is the same as $(\mathbf{Copy}(\Ab^{op}))^{op}$.
\end{example}
...
of $A$, an object of \X. $\open{A} = \{x:A\to A| x = \rst{x}\}$. The notation $\open{A}$ was chosen
...
In fact, the operation $\mathcal{O}$ is a functor, $\mathcal{O}: \X \to StabMeetSemiLat^{op}$, and
is the fundamental functor.
...
\[
  f_1(1) = a, f_1(2) \undef;\quad   f_2(1) = a, f_2(2)=b;\quad   f_3(1) = a, f_3(2)=a.
\]
Then, $f_1$ is a partial isomorphism, but is neither a partial monic nor a restriction monic. $f_2$ is a
restriction monic and a total partial isomorphism and a partial monic. $f_3$ is none of the items in
Definition~\ref{def:partial_inverse_etc}.
%todo - Find a partial monic that is not the others....
