...
For examples, consider the following maps in \Par, $f_1, f_2, f_3:\{1,2\} \to \{a,b,c\}$ where
\[
  f_1(1) = a, f_1(2) \undef;\quad   f_2(1) = a, f_2(2)=b;\quad   f_3(1) = a, f_3(2)=a.
\]
Then, $f_1$ is a partial isomorphism, but not a partial monic nor a restriction monic. $f_2$ is a
...
\begin{example}[\topcatp is not discrete]\label{ex:topcatp-not-discrete}
  ...
  To see this, for $\inv{\Delta}$ to be a continuous map, that means the diagonal must be an open
  set in the product topology of $T\times T$. This means that $\{(x,x)| x\in T\} = \union_i
  V_i\times U_i$ where $V_i$ and $U_i$ are open subsets of $T$. But this is only possible if the
  $V_i$ and $U_i$ are singletons and hence we would need the open sets in $T$ to be each element,
  i.e., $T$ must have the discrete topology.
\end{example}
