...
\begin{definition}\label{def:disjoint_sum_matrix}
  ...
  \begin{align}
    &\text{For each }i,\ \cdperp\{f_{i \jay} \ell_{\jay}\}_{\jay=1,\ldots,m}
    \text{ where }\forall  \ell_{\jay}: B_\jay \to X.
      \label{eq:disjoint_sum_matrix_row_perp}\\
    &\text{For each }j,\ \cdperp\{k_i f_{i \jay}\}_{i=1,\ldots,n}
    \text{ where }\forall  k_i : Y \to A_i.
...
\begin{proof}
  ...
  C_\ell]$. By definition, we know $h_{i k} = \djoinbig_\jay f_{i \jay} g_{\jay k}$. As each of
  $ [f_{i j}]$ and $ [g_{j k}]$ are disjoint sum matrices, we know that
  $\cdperp \{f_{i \jay} g_{\jay k}\}$. Hence, we know the composition
  $h_{i k} = \djoinbig_\jay f_{i \jay} g_{\jay k}$ is defined and $h_{i k}:A_i\to C_{k}$. By
  the stability and universality of $\djoin$, we know $\cdperp\{k_i h_{i k}\}$ and $\cdperp\{h_{i_k}
  \ell_k\}$. Thus the matrix $[h_{i k}]$ is a disjoint sum matrix and is in the category. Therefore
  ...
  $f_{i j} \inv{f_{\ell j}}$. By the definition of a disjoint sum matrix we know that for all $h$,
  we have $h f_{i j}\cdperp h f_{\ell j}$. But this means $ h f_{i j} \inv{ h f_{\ell j}} = 0$,
  giving $ h f_{i j} \inv{f_{\ell j}}\inv{h} = 0$ for all $h$, which means
  $f_{i j} \inv{f_{\ell j}} = 0$ and therefore all off-diagonal elements are $0$. The $j^{th}$
