...
\begin{proposition}\label{prop:slice-a-is-a-discrete-inverse-category}
  Given a discrete inverse category $\X$, then the restriction category $\X[A]$ defined as:
...
\end{proposition}
\begin{proof}
  Given Lemma~\ref{lem:delta_nabla_maps_are_closed}, the only remaining item to prove is that if
  $h=\dmap{h}$ then $\rst{h} = \dmap{\rst{h}}$.
