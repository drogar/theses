...
\begin{example}\label{ex:partial_map_monics_is_inverse_category}
  ...
  Section~\ref{sub:partial_map_categories}, where the stable system of monics, $\Mstab$, contains
  all of the isomorphisms in $\cP$. Then the partial isomorphisms are the maps of the form
  \raisebox{12pt}{$\xymatrix @R-15pt @C-15pt{&A'\ar[dl]_{m} \ar[dr]^{m'}\\A&&B}$}, where
  $m'\in\Mstab$. Its inverse is \raisebox{12pt}{$\xymatrix @R-15pt @C-15pt{&A'\ar[dl]_{m'}
      \ar[dr]^{m}\\B&&A}$}. The composition is \raisebox{12pt}{$\xymatrix @R-15pt
    @C-15pt{&A\ar[dl]_{m^{-1}m} \ar[dr]^{m^{-1}m}\\A&&A}$}, which is the identity when $m$ is an
  isomorphism. The partial isomorphisms will have either $m$ or $m'$ in $\Mstab$, but may not be an
  isomorphism. Taking just the partial isomorphisms gives us an inverse category.
\end{example}
...
\[
(1\*\inv{\Delta})\inv{\Delta} = \inv{(\Delta(1\*\Delta))} =  \inv{a_{\*}}(\inv{\Delta}\* 1)\inv{\Delta}.
\]
...
\begin{example}[\pinj is a discrete inverse category]\label{ex:pinj_is_a_discrete_inverse_category}
  ...
  \ref{ex:pinj_is_an_inverse_category}), we saw in Example~\ref{ex:pinj-has-a-smt} that the Cartesian
...
Example~\ref{ex:simple-slice-adjoint}. We make
