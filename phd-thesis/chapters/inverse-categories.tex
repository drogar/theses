...
when there is a map $f: A\*B \to A\*C$, but no map $g:B\to C$. We could call such maps $f$
\emph{catalytic transformations} and objects \emph{catalytic objects}.
...
\begin{enumerate}[{(}i{)}]
  ...
  \item There may be maps $f:A\+B \to A\+C$ in \X, which are not in $\X[A]$ since they do not
    preserve $A$, i.e., $f \in \dmap{A}$.
\end{enumerate}

Continuing with the inspiration of chemistry, we would argue that $\X[A]$ is perhaps a better model
of ``the catalytic transformations'' for $A$. In chemistry, one of the requirements is that
the catalyst is not changed by the transformation in any way. The theory of resource convertibility
from Definition~\ref{def:resource-convertibility} handles this by considering the resource as an
indivisible item. When one moves to a resource theory, there perhaps needs to be some way of
ensuring the catalytic object is not changed in any way. This is accomplished by ensuring the
catalytic transformation is in $\dmap{A}$.

At the same time, we question whether discrete inverse categories should be used as a resource
theory at all. The presence of the comultiplication $\Delta$ implies that any resource may be
duplicated at no cost. This seems contrary to the goal of understanding resource usage.
