...
\begin{definition}[Dagger, dagger category]\label{def:daggercat}
...
$f^{-1} = \dgr{f}$ is called \emph{unitary} and a map $f:A\to A$ with $f=\dgr{f}$ is called
\emph{self-adjoint} or \emph{hermitian}.

\begin{definition}[Dagger symmetric monoidal]\label{def:daggersmc}
...
coherence diagrams for a symmetric monoidal category. The additional coherence requirements are for
all maps $f:A\to B$ and $g:C\to D$, it is required that $\dgr{(f\*g)} = \dgr{f}\*\dgr{g}:B\*D \to
A\* C$ and that the monoid structure isomporphisms $a_{A,B,C}:(A\*B)\to C$, $u_{A}:A\to I\*A$ and
$c_{A,B}:A\*B \to B\*A$ are all unitary.


\begin{definition}[Dagger compact closed]\label{def:daggercompact}
...
\begin{proof}
  \begin{description}
    ...
    \]
    and once again apply the dagger functor. This is now the diagram defining the coproduct of maps
    and therefore $\dgr{(f\+g)} = \dgr{f}\+\dgr{g}$.
...
\[
  (f\*g) \inv{(f\*g)} = \rst{f\*g}=\rst{f} \*\rst{g} =
   f\inv{f} \* g \inv{g} = (f\*g) (\rst{f} \* \rst{g})
\]
but since the partial inverse of $f\*g$ is unique, $\rst{f\*g} = \rst{f} \* \rst{g}$. Finally,
