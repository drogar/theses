...
\begin{example}[Commutative separable Frobenius algebras]\label{example:commfrob}
...
\begin{proof}
  ...
  \ \raisebox{15pt}{.}
  \]
  %todo - Show combined with Frob condition.

  First, we must show that $\inv{f}$ is a map in the category, i.e., that $\Delta (\inv{f} \*
  \inv{f}) = \inv{f} \Delta$. We do this using string diagrams, starting from $\Delta(\inv{f} \*
  \inv{f})$.
  \[
  \begin{tikzpicture}
...
\begin{proof}
  ...
  First, we see that the tensor of $\X$ is a tensor in CFrob(\X). $A\*B$ is an object in CFrob(\X)
  with $\Delta_{A\*B} = (\Delta_A\*\Delta_B)(1\*c_{\*}\*1)$,
  $\nabla_{A\*B} =  (1\*c_{\*}\*1)(\nabla_A\*\nabla_B)$,
  $\eta_{A\*B} = \Delta_I(\eta_A \* \eta_B)$, and
  $\epsilon_{A\*B} =  (\epsilon_A\*\epsilon_B)\nabla_I$.

  The map $\Delta : A \to A\*A$ is a map in CFrob(\X). To show it preserves $\Delta$, we need to
  ...
  \raisebox{20pt}{$= \nabla_A \Delta_A$.}
  \]
  Note that the last step in the proof uses ``special'' in a non-trivial way.

  Thus, we have a $\Delta$ in CFrob(\X). As $\nabla = \inv{\Delta}$, the Frobenius requirement for
  the inverse product is immediately fulfilled. Commutativity, cocommuativity, associativity,
  coassociativity and the exchange rule all follow from the properties of the commutative Frobenius
  algebras and therefore CFrob(\X) is a discrete inverse category.
\end{proof}
