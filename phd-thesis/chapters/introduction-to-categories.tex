...
\category{All sets}{Relations: $R\subseteq X\times Y$}{$1_X = \{(x,x) | x \in X\}$}{$RS = \{(x,z) |\,
\exists y. (x,y) \in R$ and $(y,z)\in S\}$}
\end{example}


Note that \rel is enriched in posets (see Example~\ref{ex:preorders-are-categories}), via set
inclusion. \sets and \Par can be viewed as sub-categories of \rel, with the same objects, but
...
\begin{example}[\topcat]\label{ex:category_top}
  ...
  a set map $f:T \to S$ such that elements of $\open{T}$ are sent to elements of $\open{S}$. such
