...
\begin{example}\label{ex:small_finite_categories}
  ...
  \[
    \xymatrix{
      A\ar[rr]^{f} \ar@(ul,dl)[]_{id_A}& & B  \ar@(ur,dr)[]_{id_B}
    }
  \]
...
Referring to Section~\ref{ssub:functors_and_natural_transformations}, we consider some relationships
...
\begin{example}[Skeleton]\label{ex:skeleton}
   Consider the category of all finite sets, \finsets. This is equivalent to the category of ordinal
   sets, \ord, ($0 = \nothing$, $1 =
   \{0\},\ldots,n=\{0,1,\ldots,n-1\},\ldots$)\cite{maclan97:categorieswrkmath}. Take $F:\ord\to
   \finsets$ as the inclusion functor. Set $G:\finsets \to \ord$ such that $G: S \mapsto |S|$. We
   have immediately that $F G = I$ and as $\phi_S:|S|\to S$ in \finsets is an isomorphism, we have
   $\phi:G F \to I$.

   This is a specific example of a the \emph{skeleton} of a category being equivalent to the
   original category. The skeleton $\A$ of a category $\B$ is any full subcategory of $\B$ such that
   each object $B\in\B$ is isomorphic in \B to exactly one object of $\A$.
\end{example}

We now turn to adjoint functors.
...
Let us consider what happens when we have the situation above, but the $U$ in $\B$ is definable
based on the $A\in\A$.
...
Revisiting Example~\ref{ex:skeleton} above, we see that $(1,\phi):F\leftadjoint G: \ord \to
\finsets$.

\begin{example}[Simple slice adjoint.]\label{ex:simple-slice-adjoint}
  Recall the simple slice construction $\B[A]$ from Example~\ref{ex:slice_category}, where maps in
  $\B[A]$, $f:C\to D$ are maps of the form $f:C\times A \to D$ in \B.
