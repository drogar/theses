...
\begin{proof}
  ...
  \begin{enumerate}[{(}i{)}]
    ...
    \item We are given there is an $h$ such that $(f g) h = 1$. This means $f (g h) =1$ and $f$ is
      a section.
...
\begin{proof}
  ...
  \categoryns{Categories.}{Functors.}{The identity functor which takes an object to the same object
    ...
    $F G(x) = G(F(x))$. This is clearly associative.}
  Note \cat is often regarded as a \emph{large} category, as its collection of objects is not a set.
\end{proof}
...
We now have the machinery to discuss the relationship between a category \B and its splitting
$\spl{}{\B}$:
...
\begin{lemma}\label{lem:adjoint-setup}
  Suppose $G:\B\to\A$ is a functor such that for each $A\in\objects{\A}$, there is a map on objects
...
inclusion. \sets, \Par and \pinj can be viewed as subcategories of \rel, with the same objects, but
restricting the maps allowed --- see each example below. \Par is also enriched in posets, via the
...
\begin{example}[\topcat]\label{ex:category_top}
  ...
  \begin{enumerate}[{(}i{)}]
    ...
    \item The union of any sets in $\open{T}$ is in $\open{T}$;
...
\end{figure}
\begin{example}
  Any category \D with a product, $\times$, and a terminal object, $\top$, is a symmetric monoidal
