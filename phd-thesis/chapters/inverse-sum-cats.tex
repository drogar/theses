...
\begin{definition}\label{def:distributive_inverse_category}
  ...
  \begin{itemize}
    ...
    \end{equation}
    commutes in \D for any choices of objects $A, B, C$, where the map $d$ is an isomorphism.
  \end{itemize}
\end{definition}
...
\begin{proof}
  ...
  Note this corresponds to the conditions of
  Lemma~\ref{lem:maps_from_inverse_sums_have_disjoint_equivalences}. Hence by that lemma we have an
  $f'$ and $g''$ in \X such that $\icpa f' \perp \icpb g''$ and we may therefore form the disjoint
  join. We set $h$ in \Xt to be $(\icpa f' \djoin \icpb g'', (C\+D))$.
