...
\begin{proof}
  ...
  composition. In actuality, the proof of closure only relies on one of the maps being in
  $\dmap{T}$. Thus, if $h\in \dmap{T}$ then $f h g$ will be in $\dmap{T}$ as well.
  ...
  \[
    \xymatrix @C=15pt @R=15pt{
      & & & T \* T \ar@{.>}[d]^{r_{T\*C}}\\
      & & T \* T \* C \ar@{.>}[dddd]^{h'_f} \ar[ur]^{m_{T\*C}} \ar@{=}[r] &T\*T\*C \ar@{.>}[dddd]^{h'_f} \\
      &T\*T \ar[ur]^{\circ} & & \\
      T \ar[ur]^(.3){\inv{\usl}(\code{f}\*1)} \ar[ddrr]_{f} \\
      & & & \\
      && T\*D \ar[dr]_{m_D} &  T\*D \ar[d]_{m_D}\\
      &&&T\*T.
    }
  \]
  But this then is the required diagram for an inverse Turing category with $h_f = r_{T\*C} h' f m_D$
  and therefore $\Inv{T}$ is an inverse Turing category with $\diamond = \circ\, m_{T\*C}$.
\end{proof}
