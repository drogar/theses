...
Turing categories will provide the link to standard models of computability as promised in our
introduction. A Turing category may give a categorical formulation of various models of
computability, including partial combinatory algebras and the partial lambda calculus as shown in
\cite{cockett-hostra08-intro-to-turing}. Providing a link from inverse categories with the
constructions we derive below to Turing categories provides the link between reversible computation
and standard computation which we originally set out to show.
...
\begin{lemma}\label{lem:discrete_turing_category_inverses_make_inverse_turing_category}
  When $T$ is a Turing category that is also a discrete Cartesian restriction category, then
  $\Inv{T}$ is an inverse Turing category.
\end{lemma}
\begin{proof}
  First, we immediately know that $\Inv{T}$ is an inverse category. Additionally, we know that the
  map $\<\iname{f},1\>$ is invertible as we are in a discrete Cartesian restriction category.
  Given $f$ is invertible, we know that we have the diagram
      \[
        \xymatrix{
          T\times T \ar[r]^{\circ} & T \\
          T \ar[ur]_{f} \ar[u]^{\<!\code{f},1\>}
      ...
      \]
  in $T$. Translating this directly to $\Inv{T}$, for some $h'_f \in \dmap{T}$ we have:
  \begin{equation}
    \xymatrix @C=15pt @R=15pt{
      & & T \* T\*C \ar@{.>}[dddd]^{h'_f}\\
      &T\*T \ar[ur]^{\circ} & & \\
  ...
  Next, note that for any monic $m$, we have $f \xequiv{1\*m} f m$. We have both $T\*C \retract T$
  and $D \retract T$, and as equivalence is a transitive relation, we may redraw
  Diagram~\ref{dia:pre-inverse-turing} as
  \[
    \xymatrix @C=15pt @R=15pt{
      & & T \* T \ar@{.>}[dddd]^{h_f}\\
      &T\*T \ar[ur]^{\circ\, m_{T\*C}} & & \\
      T \ar[ur]^(.3){\inv{\usl}(\code{f}\*1)} \ar[ddrr]_{f m_D} \\
      & & & \\
      && T\*T.
    }
  \]
  But this is the required diagram for an inverse Turing category and therefore $\Inv{T}$ is an
  inverse category with $\diamond = \circ\, m_{T\*C}$.
\end{proof}

We know from Chapter~\ref{chap:completing_a_discrete_inverse_category} that when \X is an inverse
Turing category, we know \Xt is a discrete Cartesian restriction category. Moreover, if
$A\retractmaps{m_A}{r_A} T$ in \X, then $A \retractmaps{m_A\inv{\usr}}{r_A\inv{\usr}} T$ in \Xt and hence $T$
...
self-application map and a code for each map $f:1\to T$ as in
...
\begin{proof}
  ...
  where $B\retractmaps{m_B}{r_B} T$ in \X, we see $(f,B) \xequiv{} (fr_B,T)$ in \X and therefore may be
...
category by Lemma~\ref{lem:discrete_turing_category_inverses_make_inverse_turing_category}. However,
we can not guarantee that \compa is a discrete Turing category.  However, we can define conditions
so that it is true:
