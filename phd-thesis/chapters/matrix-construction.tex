...
\cite{haghverdi:2000phd} gives a formula for computing the trace, when it exists. In a unique
...
\[
  ...
  \end{bmatrix}
\]
the trace is given by the formula:
\begin{equation}
  Tr^{U}_{X,Y}(f) = f_{11} +\sum_{n=0}^{\infty}f_{12}f_{22}^n f_{21},\label{eq:trace}
\end{equation}
 whenever this sum exists.

Therefore, to show an inverse category with a disjoint sum has a trace, we would simply need to show
the conditions for which the sum in Equation~\ref{eq:trace} exists.
