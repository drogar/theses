...
\begin{definition}\label{def:category_alt}
  ...
  there is a map called the \emph{composite} of $f$ and $g$, written $f g$ such that $f g:X \to Z$.
...
\begin{proof}
  ...
  and the co-domain is $C(f)$. By \catthree, for $f:X\to Y$ and $g:Y \to Z$ the composite $f g$ is
...
\begin{proof}
  ...
  \]
  Then, substituting $\pi_1$ for $\pi_0$ above, gives us $f \cap g \le g$.
...
\begin{definition}\label{def:discrete_restriction_category}
  A  Cartesian restriction category in which every object is
...
\begin{proof}
  ...
  %     \rst{(1 \times f)} \pi_0, \rst{(1 \times f)\inv{\Delta}}\pi_1 \>\\
    ...
    &= \rst{(1 \times f)\inv{\Delta}}
  \end{align*}
  The above follows as in a discrete restriction category, we have
  \begin{align*}
    \rst{\inv{\Delta}} \pi_1
...
\input{chapters/section/frobenius-algebra-definition}
\input{chapters/section/bases-and-frobenius}
