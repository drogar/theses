...
\begin{definition}\label{def:split_category}
  Given a category $\B$ and a set of idempotents $E$ of $\B$, we define \emph{Split${}_E$($\B$)},
  written as $\spl{E}{\B}$, as the following category:
...
\begin{definition}\label{def:categorical_biproduct}
  In a category \B, given any two objects, $A,B$, if $A\times B$ is isomorphic to $A+B$,  it is
...
\end{lemma}
\begin{proof}
  \categoryns{Categories.}{Functors.}{The identity functor which takes a map to the same map.}{
  $F G(x) = F(G(x))$ which is clearly associative.}
\end{proof}

We may also consider the notion of containment between categories, via functors.

\begin{definition}\label{def:subcategories}
  Given the categories $\B$ and $\D$, we may say the following:
  \begin{enumerate}[{(}i{)}]
    \item \B is a \emph{sub-category} of \D when each object of \B is an object of \D and when each map
    of \B is a map of \D.
    \item \B is a \emph{full sub-category} of \D when it is a sub-category and given $A, B$ objects
      in $\B$ and $f:A \to B$ in \D, then $f$ is a map in $\B$.
  \end{enumerate}
\end{definition}
...
A functor $F:\B \to \D$ induces a map between hom-objects in $\B$ and hom-objects in $\D$. For
...
\begin{definition}\label{def:full_functor_faithful_functor}
  Given a functor $F:\B \to \D$, we say:
  \begin{itemize}
...
\begin{definition}\label{def:discrete_object}
