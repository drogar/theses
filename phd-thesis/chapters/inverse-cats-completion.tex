...
\begin{definition}[Cartesian Completion]\label{def:xt}
  When \X is an inverse category, define \Xt, the Cartesian Completion of \X, as:
...
Suppose $f\xequiv{h}g$. Then $f\xequiv{\rg{f}h\rg{g}} g$.
\end{lemma}
\begin{proof}
  We know that $h=\dmap{h}$. But by Lemma~\ref{lem:delta_nabla_maps_are_closed}, we may pre-compose
  and post-compose with restriction idempotents, thus, $\rg{f}h\rg{g} = \dmap{(\rg{f}h\rg{g})}$. By
...
\begin{corollary}\label{cor:inv-f-g-provides-equivalence}
  Given $f, g$ in a discrete inverse category, if $\rst{f} = \rst{g}$ and $\inv{f} g =
  \dmap{(\inv{f} g)}$, then $f \xequiv{} g$.
\end{corollary}
...
\begin{theorem}\label{thm:xt_is_a_discrete_crc_when_x_is_an_inverse_category}
  For any inverse category \X, the category \Xt is a discrete Cartesian restriction category.
\end{theorem}
\begin{proof}
