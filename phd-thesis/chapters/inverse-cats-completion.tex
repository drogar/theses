...
\chapter{Constructing a Cartesian restriction category from a discrete inverse category} % (fold)
...
\begin{definition}[Cartesian Completion]\label{def:xt}
...
Although Definition~\ref{def:xequivalence} above does not require a unique $h$, we may always find a
minimal such $h$.
...
\begin{proof}
  ...
  \]
  Thus, by transitivity  $\inv{f}g \le h$ and is minimal among all $h$ such that $f\xequiv{h} g$.
\end{proof}
...
\begin{proof}
  ...
  Lemma~\ref{lem:mediating_map_equivalence_is_symmetric_reflexive_and_transitive}. As \X is a
  discrete inverse category, $\inv{\usr}$  is defined and therefore the $(\inv{\usr}, 1)$ is a map
  in \Xt.
  ...
  by Lemma~\ref{lem:restrictionvarious} and $\inv{\usr}$ is natural. The diagram
  \[
    \xymatrix @C=50pt @R=45pt{
  ...
  Hence we have shown both Equations~\ref{eq:cc-identity-1} and~\ref{eq:cc-identity-2}
  hold and therefore $(\inv{\usr},1)$ is the identity map in \Xt.
\end{proof}

Now that we have shown \Xt is a category, we may define a restriction:
...
\begin{proof}
  ...
  \rone ($\restr{(f,C)} (f,C) = (f,C)$). Calculating the restriction of the left hand side in
    ...
    \rthree ($\restr{\restr{(f,C)} (g,D)} = \restr{(f,C)} \restr{(g,D)}$ ). We must show
      ...
      bottom curved arrow, using the fact that
...
\begin{proof}
  ...
  \begin{enumerate}[{(}i{)}]
    \item The maps $\pi_0$ and $\pi_1$ are total; \label{item:xt-product-1}
    \item $\rst{\<(f,C),(g,D)\>} = \rst{(f,C)}\,\rst{(g,D)}$; \label{item:xt-product-2}
    \item $\<(f,C),(g,D)\> \pi_0 \le (f,C)$ and $\<(f,C),(g,D)\> \pi_1 \le (g,D)$. \label{item:xt-product-3}
  \end{enumerate}
  ...
  Thus, to show the equality of \ref{item:xt-product-2} in \Xt, we need only show that
  $\rst{f}\,\rst{g}\inv{\usr}$ is equivalent to $\rst{f} \rst{g}\inv{\usr}\inv{\usr} a_{\*}$ in \X.
  The restriction of both of these, in \X, is $\rst{\rst{f} \rst{g}}$, via
  Lemma~\ref{lem:restrictionvarious}. Thus, using the mediating map $1\*\usr$, we have
  \[
    \rst{f}\,\rst{g}\inv{\usr}\xequiv{1\*\usr}\rst{f} \rst{g}\inv{\usr}\inv{\usr} a_{\*}.
  \]
  This shows $\rst{\<(f,C),(g,D)\>}=\rst{(f,C)} \rst{(g,D)}$ in \Xt.


  Finally, for \ref{item:xt-product-3}, to show $\<(f,C),(g,D)\> \pi_0 \le (f,C)$ (and
  $\<(f,C),(g,D)\> \pi_1 \le (g,D)$), it is required to show
  $\rst{\<(f,C),(g,D)\>\pi_{0}} (f,C) = \<(f,C),(g,D)\> \pi_{0}$. Calculating the left side, we see:
  \begin{align*}
    \rst{\<(f,C),(g,D)\>\pi_{0}} (f,C) &=\rst{\<(f,C),(g,D)\>\rst{\pi_{0}}} (f,C) &\text{Lemma \ref{lem:restrictionvarious}}\\
    &=\rst{\<(f,C),(g,D)\>} (f,C) &\pi_{0}\text{ is total}\\
    &=\rst{(f,C)}\,\rst{(g,D)}\, (f,C)&\text{ by above}\\
    &=\rst{(g,D)} \rst{(f,C)} (f,C) & \rtwo\\
    &=\rst{(g,D)} (f,C)& \rone.
  \end{align*}
  Now, turning to the right hand side:
  \begin{align*}
    \<(f,C),(g,D)\>\pi_{0} &= \Delta(f\*g)(1\*c_{\*}\*1) 1 &\text{in \X, by definition.}
  \end{align*}
  To show these are equal in \Xt, we need to first show the
  restrictions are the same in \X and
  then give a mediating map. For $\rst{(g,D)} (f,C)$, in \X, this is $\rst{g}\inv{\usr}(f\*1)$.
  Calculating the restriction, we have
  \[
    \rst{\rst{g}\inv{\usr}(f\*1)} = \rst{\rst{g} f \inv{\usr}} = \rst{\rst{g} f} = \rst{f}\rst{g}.
  \]
  For the right hand side, calculate in \X:
  \begin{align*}
...
\begin{proof}
  ...
  \]
\end{proof}
\section{Equivalence between the category  {\bf DInv} and the category {\bf DCartRest}} % (fold)
...
\]
\caption{Functors between Cartesian restriction categories and inverse
  categories.}\label{fig:dcartrest-to-cartrest-to-dinv}
\end{figure}

Thus, we may now draw out the relationship between Cartesian restriction categories, discrete
Cartesian restriction categories and discrete inverse categories in
Figure~\ref{fig:dcartrest-to-cartrest-to-dinv}. In the figure, the arrow from discrete Cartesian
restriction categories to Cartesian restriction categories is the standard embedding and the reverse
arrow picks out the discrete objects in the Cartesian restriction category. Of course, the terminal
object is always discrete, as noted in Example~\ref{ex:terminal_object_is_discrete}.

% subsection equivalence_of_categories (end)
\section{Examples of the Cartesian Construction} % (fold)
...
\end{align*}
This shows $\widetilde{\D}$ is  the straight-line ($a \to b \to c \to d$) lattice. The tensor in \D
becomes the meet and hence is a categorical product in $\widetilde{\D}$. Note that the only partial
inverses in $\widetilde{\D}$ are the identity functions and that for all maps $f$, $\<f,1\> = id$.
...
\begin{align*}
  ...
  d \xrightarrow{(id,d)} d.
\end{align*}
In this case, $\widetilde{\D}$ results in the ``diamond'' lattice, \raisebox{19pt}{
$
