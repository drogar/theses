...
\chapter{Creating a Cartesian restriction category from a discrete inverse category} % (fold)
...
\begin{proof}
  ...
  \]
\end{proof}
\section{Discrete inverse categories are equivalent to discrete Cartesian restriction categories} % (fold)
...
\begin{lemma}\label{lem:universal_diagram_is_a_commutative_diagram}
  ...
  That is, $\eta\Inv{F^{\#}} = F$ and $F^{\#}$ is unique.
\end{lemma}
\begin{proof}
  ...
  \[
    \xymatrix @C=45pt @R=25pt{
      B \ar[r]^{k} \ar[rrddd]_{\rst{k}\inv{\usr}}
        & A \*D \ar[r]^{g\*1}&B\*C\*D \ar[d]^{\Delta\*1}\\
      & &  B\*B\*C\*D \ar@{.>}[d]^{1\* n} \\
