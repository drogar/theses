...
\begin{proof}
  ...
  We may then form the map $q_1(1\*\cpa) \tjdown q_2(1\*\cpb): Y\*E \to Y\*(C\+D)$.  NOT TRUE, NEED
  Rst TO BE  $\perp$, WORK THIS PART OUT.

% By the
%   universal property of disjointness,
%   Lemma~\ref{lem:disjointness_various}~\ref{lemitem:djv_universal}, we have
%   \[\wrg{q_1(1\*\cpa)}(\inv{\Delta}\*1) \perp \wrg{q_2(1\*\cpb)}(\inv{\Delta}\*1).\] By
%   \axiom{Dis}{6}, we then have
%   \[\wrg{\wrg{q_1(1\*\cpa)}(\inv{\Delta}\*1)} \perp \wrg{\wrg{q_2(1\*\cpb)}(\inv{\Delta}\*1)}.\] But
%   by \rrfour ($\wrg{\rg{f}g} = \wrg{f g}$), we now have
%   \[\wrg{q_1(1\*\cpa)(\inv{\Delta}\*1)} \perp \wrg{q_2(1\*\cpb)(\inv{\Delta}\*1)}.\]
