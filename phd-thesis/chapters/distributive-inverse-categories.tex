...
\begin{proof}
  ...
  \begin{align*}
    \cpa k &\xequiv{q_1} f\quad\text{and}\\
    \cpb k &\xequiv{q_2} g.
  \end{align*}
  where the maps $q_1: Y\*E \to Y\*C$ and $q_2:Y\*E \to Y\*D$ fulfill the respective equivalence diagrams.
  ...
  \[
    \xymatrix @C=20pt @R=20pt{
      & & Y \* E \ar@{.>}[dr]^{\Delta\* 1}\\
      & & & Y \* (Y\* E) \ar@{.>}[dd]^{1\*q_1}\\
      A \ar[uurr]^{\cpa k} \ar[ddrr]_{f} \\
      & & & Y \* Y\*C \ar@{.>}[dl]^(.4){\inv{\Delta}\* 1}\\
      && Y\*C.
    }
  \]
  ...
  we know that \[\wrg{1\*\icpa}\perp \wrg{1\*\icpb}\]. By
  Lemma~\ref{lem:ordering_of_restriction_and_range}~\ref{lemitem:ordering_1} ($\wrg{f h} \le
  \rg{h}$) and \axiom{Dis}{3}, we have \[\wrg{q_1(1\*icpa)} \perp \wrg{q_2(1\*\icpb)}.\] By the
  universal property of disjointness,
  Lemma~\ref{lem:disjointness_various}~\ref{lemitem:djv_universal}, we have
  \[\wrg{q_1(1\*icpa)}(\inv{\Delta}\*1) \perp \wrg{q_2(1\*\icpb)}(\inv{\Delta}\*1).\] By
  \axiom{Dis}{6}, we then have
  \[\wrg{\wrg{q_1(1\*icpa)}(\inv{\Delta}\*1)} \perp \wrg{\wrg{q_2(1\*\icpb)}(\inv{\Delta}\*1)}.\] But
  by \rrfour ($\wrg{\rg{f}g} = \wrg{f g}$), we now have
  \[\wrg{q_1(1\*icpa)(\inv{\Delta}\*1)} \perp \wrg{q_2(1\*\icpb)(\inv{\Delta}\*1)}.\]

  We are now in a situation where we may apply \axiom{Dis}{7} OH CRUD I WENT THE WRONG WAY AROUND!!!!
