...
product distributes over the disjoint sum in a specific way. This chapter will show that the
...
\begin{lemma}\label{lem:distributive_means_distribute_over_join}
  Given an inverse distributive category \X, $\*$ distributes over the disjoint join.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \end{enumerate}
\end{proof}
\begin{theorem}\label{thm:x_tilde_has_coproducts_if_x_is_inverse_distributive_category}
