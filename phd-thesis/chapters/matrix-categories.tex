...
\begin{lemma}\label{lem:inverse_matrix_category_is_a_restriction_category}
  When $\X$ is an inverse category with enough disjoint sums, \imatx is an inverse category.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  an disjoint sum of $A$ and $B$ for any two objects in \imatx, and hence, \imatx is an disjoint
  sum category.
\end{proof}

\section{Equivalence between an disjoint sum category and its matrix category} % (fold)
\label{sec:equivalence_between_an_disjoint_sum_category_and_its_matrix_category}
In this section we will provide restriction functors between an disjoint sum category
